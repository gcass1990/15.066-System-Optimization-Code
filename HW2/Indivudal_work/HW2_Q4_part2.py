...
open_vars = {} #intailize, holds decision about whether or not to open facility
...
for i in facilities:
    open_vars[i] = m.addVar(vtype=gp.GRB.BINARY, name=i)  # This facility is opened or not
    # Calculate cost of making this decision to open the facility
    open_cost = open_cost + facility_opening_costs[i] * open_vars[i]

    for j in services:
        open_serv[(i,j)] = m.addVar(vtype=gp.GRB.BINARY, name=i+j)  # This facility is opened or not
        serv_cost = serv_cost + open_serv[(i,j)]*service_installation_costs[i][j]
        #Constraint that


        for k in customers:
            dv_name = i+"-"+j+"-"+k
            decision_vars[(i,j,k)] = m.addVar(vtype=gp.GRB.BINARY, name=dv_name) #This path is chosen or not

            #Calculate cost of making the decision to connect customer,service, and facility

            ass_cost = facility_customer_assignment_costs[i][k]
            #print(i,j,k, serv_cost,ass_cost)
            connection_cost = connection_cost + (ass_cost*decision_vars[(i,j,k)])

            #Add constraint that can only make the connection if the facility is open
            #3*7*4 = 84 constraints total
            m.addConstr(open_vars[i] >=decision_vars[(i,j,k)])
...
for i in facilities:
    ...
    for j in services:
        for k in customers:
            numberserved = numberserved + decision_vars[(i,j,k)]
    m.addConstr(numberserved <= 4)
