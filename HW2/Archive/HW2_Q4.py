...
for i in facilities:
    open_vars[i] = m.addVar(lb=0, ub=1, vtype=gp.GRB.INTEGER, name=i)  # This facility is opened or not
    # Calculate cost of making this decision to open the facility
    open_cost = open_cost + facility_opening_costs[i] * open_vars[i]

    for j in services:
        open_vars[i] = m.addVar(lb=0, ub=1, vtype=gp.GRB.INTEGER, name=i+j)  # This facility is opened or not
...
total_cost=  open_cost +connection_cost
#print(decision_vars)
#print(len(decision_vars))
#print(total_cost)

#Set up constraints that service must be provided to the customer from one o the three facilities
#7 constraints total
